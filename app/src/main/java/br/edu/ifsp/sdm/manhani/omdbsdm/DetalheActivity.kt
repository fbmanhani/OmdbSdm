...
class DetalheActivity : AppCompatActivity() {
    ...
    private fun recuperarFilmePorId() {
        omdbApi.getFilme(id).enqueue(
            object : Callback<RetornoDetalhe> {
                override fun onFailure(call: Call<RetornoDetalhe>, t: Throwable) {
                    mainContent?.snackbar("Erro: " + t.message)
                }

                override fun onResponse(call: Call<RetornoDetalhe>, response: Response<RetornoDetalhe>) {
